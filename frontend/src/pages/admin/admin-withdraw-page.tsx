...
import { extractApiError, withdraw } from '../../api';
...
export function AdminWithdrawPage() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    setMessage('');
    try {
      const result = await withdraw(accountNumber, Number(amount));
      setMessage(`Withdraw completed. New balance: ${result.balance}`);
    } catch (err) {
